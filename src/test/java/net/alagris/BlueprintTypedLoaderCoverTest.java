...
public class BlueprintTypedLoaderCoverTest {
	...
	public void parsing() throws Exception {
		...
		assertArrayEquals("Paths wrong!", new String[] { "tap1", "tap2" }, blueprint.getGlobal().get("paths", String[].class));
		
		HashMap<String, Node> nodes = blueprint.collectById();
		Node preprocessor = nodes.get("Preprocessor-id");
		HashMap<String, String> preprocessorC = preprocessor.getConfig();
	...
	@Test
	public void process() {
		Pair[] testSet = new Pair[] { new Pair("", "-NEW"), new Pair("a", "A-NEW"), new Pair("hello world", "HELLO WORLD-NEW"),
				new Pair("6237584", "6237584-NEW"), new Pair("_^$@(%#", "_^$@(%#-NEW"), };
		for(Pair p : testSet) {
